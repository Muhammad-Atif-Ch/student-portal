...
@section('title', __('Add Exam Pool Rule'))
@section('content')
    <!-- Main Content -->
            ...
            <div class="section-body">
                <div class="row">
                    <div class="col-12 col-md-12 col-lg-12">
                        <div class="card">
                            <form action="{{ route('admin.exam.pool-rule.store') }}" method="POST">
                                @csrf
                                <div class="card-header">
                                    <h4>Create Exam Pool Rule</h4>
                                </div>
                                <div class="card-body">
                                    <div class="row">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select class="form-control" name="exam_type_id" required>
                                                    ...
                                                    @foreach ($examTypes as $examType)
                                                        <option value="{{ $examType->id }}" {{ old('exam_type_id') == $examType->id ? 'selected' : '' }}>{{ $examType->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-12 col-md-4 col-lg-4">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control" name="pool_type" id="pool_type" required>
                                                    <option value="" selected disabled>Select Pool Type</option>
                                                    @foreach (['common', 'specific'] as $poolType)
                                                        <option value="{{ $poolType }}" {{ old('pool_type') == $poolType ? 'selected' : '' }}>{{ ucfirst($poolType) }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                    ...
                                    <div class="row">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select class="form-control" name="specific_type" id="specific_type">
                                                    ...
                                                    @foreach (['car', 'bike', 'bus', 'truck'] as $type)
                                                        <option value="{{ $type }}" {{ old('specific_type') == $type ? 'selected' : '' }}>{{ ucfirst($type) }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        <div class="col-12 col-md-4 col-lg-4">
                                            <div class="form-group">
                                                ...
                                                <input type="number" name="required_count" class="form-control" value="{{ old('required_count') }}" min="1" required>
                                            </div>
                                        </div>
                                    </div>
                                </div>
